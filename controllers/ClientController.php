...
class ClientController {
   ...
   /**
     * Obtém lista completa de lojas parceiras para o hub (SEM SALDO)
     * 
     * @param int $userId ID do cliente
     ...
     * @return array Lista de lojas parceiras
     */
    public static function getPartnerStoresHub($userId, $filters = [], $page = 1) {
        try {
            // Verificar se é um cliente válido
            if (!self::validateClient($userId)) {
                return ['status' => false, 'message' => 'Cliente não encontrado ou inativo.'];
            ...
            // Consulta base (CORRIGIDA para evitar duplicatas)
            $query = "
                SELECT DISTINCT 
                    l.id,
                    l.nome_fantasia,
                    ...
                    CASE WHEN DATEDIFF(NOW(), l.data_cadastro) <= 30 THEN 1 ELSE 0 END as is_new
                FROM lojas l
                LEFT JOIN favoritas f ON (f.loja_id = l.id AND f.usuario_id = :user_id)
                WHERE l.status = :status
            ";
            ...
            if (!empty($filters)) {
                ...
                // Filtro por nome
                if (isset($filters['nome']) && !empty($filters['nome'])) {
                    $query .= " AND l.nome_fantasia LIKE :nome";
                    $params[':nome'] = '%' . $filters['nome'] . '%';
                }
                ...
                if (isset($filters['favoritas']) && $filters['favoritas']) {
            ...
            // Aplicar ordenação
            $orderBy = " ORDER BY ";
            if (isset($filters['ordenar'])) {
                switch ($filters['ordenar']) {
                    case 'cashback_desc':
                        $orderBy .= "l.porcentagem_cashback DESC, l.nome_fantasia ASC";
                        break;
                    case 'categoria':
                        $orderBy .= "l.categoria ASC, l.nome_fantasia ASC";
                        break;
                    case 'recentes':
                        $orderBy .= "l.data_cadastro DESC, l.nome_fantasia ASC";
                        break;
                    default:
            ...
            $countQuery = "SELECT COUNT(DISTINCT l.id) " . substr($query, strpos($query, 'FROM'));
            ...
            $statsQuery = "
                SELECT 
                    ...
                    AVG(porcentagem_cashback) as cashback_medio,
                    (SELECT COUNT(*) FROM favoritas WHERE usuario_id = :user_id) as minhas_favoritas
                FROM lojas 
                WHERE status = :status
            ";
            ...
            error_log('Erro ao obter lojas parceiras hub: ' . $e->getMessage());
            return ['status' => false, 'message' => 'Erro ao carregar lojas parceiras. Tente novamente.'];
        }
    } 
