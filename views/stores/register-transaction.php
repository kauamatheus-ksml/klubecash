        ...
        if ($userQuery->rowCount() === 0) {
            ...
            if (empty($error)) {
                ...
                if ($result['status']) {
                    ...
                    // === INTEGRAÇÃO WHATSAPP DIRETA (COM TRATAMENTO MELHORADO) ===
                    try {
                        ...
                        if ($notificationResult && isset($notificationResult['success']) && $notificationResult['success']) {
                            error_log("[TRACE] register-transaction.php - Notificação enviada com sucesso: " . json_encode($notificationResult), 3, '../../integration_trace.log');
                        } else {
                            error_log("[TRACE] register-transaction.php - Notificação falhou (não crítico): " . json_encode($notificationResult), 3, '../../integration_trace.log');
                            // Não exibir erro para o usuário pois a transação foi criada com sucesso
                        }
                    } catch (Exception $e) {
                        error_log("[TRACE] register-transaction.php - ERRO na notificação (não crítico): " . $e->getMessage(), 3, '../../integration_trace.log');
                        // Falha na notificação não deve afetar a transação principal
                    }
